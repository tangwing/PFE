...
void ConstructCut2()
{
		...
		printf("[Info] Declaration of Cut2 : SEUIL=%d\n", NB_1CUT_SEUIL);
		int iLoop,iLoop2,iLoop3,iLoop4;
		res.nbConCut2=0 ;
		 //1-cuts for constraint A/BC/D
		vector<Equation> vEquations;
		vEquations.reserve(T()*M()*4);//

		for (iLoop=0;iLoop<T();iLoop++)
		 for (iLoop3=0;iLoop3<M();iLoop3++)
		 {//Loop for constructing each equation from which we will make 1-cuts
			 shared_ptr< vector<Term> > vEquationA( new vector<Term>() ); //CPU
			 shared_ptr< vector<Term> > vEquationB( new vector<Term>() ); //GPU
			 shared_ptr< vector<Term> > vEquationC( new vector<Term>() ); //HDD
			 shared_ptr< vector<Term> > vEquationD( new vector<Term>() ); //RAM
			...
			//Now we have 4 equations, add to vector
			vEquations.push_back( make_pair( vEquationA, mc(iLoop3)));
			if(mg(iLoop3)>0)vEquations.push_back( make_pair( vEquationB, mg(iLoop3)));
