...
// int num,ri_temp;
 ...
 for (iLoop=0;iLoop<Data.NbTasks;iLoop++)
 {
	 for (iLoop2=0;iLoop2<Data.TimeHorizon;iLoop2++) // We read the variables uit: 1 if task i has to be processed at time t
		fscanf_s(file,"%hd ",&Data.ListOfTasks[iLoop].LIsToBeProcessed[iLoop2]); 
	 fscanf_s(file,"\n");
 }
 ...
 for (iLoop=0;iLoop<Data.NbTasks;iLoop++)
 {
	 for (iLoop2=0;iLoop2<Data.NbTasks;iLoop2++) 
			 fscanf_s(file,"%hd ",&Data.ListOfTasks[iLoop].LAffinities[iLoop2]); 
	 fscanf_s(file,"\n");
 }
 ...
 for (iLoop=0;iLoop<Data.NbTasks;iLoop++)
 {
	 for (iLoop2=0;iLoop2<Data.NbMachines;iLoop2++) 
			 fscanf_s(file,"%hd ",&Data.ListOfTasks[iLoop].LPreAssignement[iLoop2]); 
	 fscanf_s(file,"\n");
 }
 ...
 {
	 for (iLoop2=0;iLoop2<Data.NbTasks;iLoop2++) 
			 fscanf_s(file,"%hd ",&Data.Network.ComBandwidth[iLoop][iLoop2]); 
	 fscanf_s(file,"\n");
 }
 // We set the bii into the matrix ComBandwidgth
  ...
  {
	  fscanf_s(file,"%hd ",&Data.Network.NbMachinesByEdge[iLoop]);
	  for (iLoop2=0;iLoop2<Data.Network.NbMachinesByEdge[iLoop];iLoop2++)
		  fscanf_s(file,"%hd %hd ",&Data.Network.ListOfMachinesByEdge[iLoop][iLoop2][0],&Data.Network.ListOfMachinesByEdge[iLoop][iLoop2][1]);
	  fscanf_s(file,"\n");
  }
 fclose(file);
