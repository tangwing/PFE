...
//Add a new line in the statistics table of Cplex & H1
void MakeStatCplexH1(int IdSce, int NbTache, int NbMach, int NbIter)
{
	...
	}else
	{
	    ...
	    fichier=fopen("StatCplexH1.csv","at");
	}
...
//Add a new line in the statistics table for Cplex & H2
void MakeStatCplexH2(int IdSce, int NbTache, int NbMach, int NbIter)
{
	static bool printHeader = true;
	int NbResE = 0, 
	...
	for(int i=0; i<NbIter; i++)
	{
		...
		///Heuristic2
		if(ScRes[2][i][0] == 1) {NbResH++; if(ScRes[2][i][1] == 1) NbOptH++;}
		TTotalH += ScRes[2][i][3];
	...
	}else
	{
	    ...
	    fichier=fopen("StatCplexH2.csv","at");
	}
	fprintf(fichier,"Sc%d(%d/%d); %d; %d; %d; %d; %3.2lf; %3.2lf; %3.2lf; %d; %d; %3.2lf; %3.2lf; %3.2lf",
		IdSce+1, NbTache, NbMach, 
		iterations - NbOptE - NbMemLimitE - NbTimeLimitE, //We calculate #infea like this because some infeasible situations are caused by *Lim and we count this as *Lim. 
